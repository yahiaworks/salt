...
string DOCKER_REPO = "docker-development.docker.vips.vistaprint.io"
        ...
        stage('Pushing...') {
            if (getBranchType() == "MASTER") {
                withCredentials([usernamePassword(credentialsId: 'd262c113-387e-4a85-bedb-133ced13259b',
                    ...
                    runShellStep("jenkins/docker_push.sh", "publish", "$DOCKER_REPO $version $ARTIFACTORY_USER $ARTIFACTORY_PASSWORD")
                }
            }
        }
        stage('Deploying...') {
